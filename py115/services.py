...
class StorageService:
    ...
    def upload_hash(self, dir_id: str, file_path: str, file_size: str, file_hash: str) -> types.UploadTicket:
        file_name = os.path.basename(file_path)
        with open(file_path, 'rb') as file_io:
            return self.request_hash_data(dir_id, file_name, file_size, file_hash, file_io)
    
    def request_hash_data(
            self, 
            dir_id: str, 
            save_name: str, 
            file_size: str,
        ...
        if not (data_io.readable() and data_io.seekable()):
            return None
        init_result = self._client.execute_api(upload.InitApiHash(
            target_id=f'U_1_{dir_id}',
            file_name = save_name,
            file_size = file_size,
            file_hash = file_hash,
